...
The **`search`** property of the {{domxref("SVGAElement")}} interface is a search string, also called a _query string_, that is a string containing a `"?"` followed by the parameters of the SVG {{SVGElement("a")}} element's {{SVGAttr("href")}}. If the URL does not have a search query, this property contains an empty string, `""`.

This property can be set to change the query string of the URL. When setting, a single `"?"` prefix is added to the provided value, if not already present, and setting it to `""` removes the query string. Setting this property updates the element's {{SVGAttr("href")}} attribute with the serialized, absolute URL.
