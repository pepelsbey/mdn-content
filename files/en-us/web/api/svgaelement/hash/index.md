...
This property can be set to change the fragment of the URL. When setting, a single `"#"` prefix is added to the provided value, if not already present, and setting it to `""` removes the fragment. Setting this property updates the element's {{SVGAttr("href")}} attribute with the serialized, absolute URL.
