...
This property can be set to change the username of the URL. If the URL has no {{domxref("SVGAElement.host", "host")}} or its scheme is `file:`, then setting this property has no effect. Setting this property updates the element's {{SVGAttr("href")}} attribute with the serialized, absolute URL.
