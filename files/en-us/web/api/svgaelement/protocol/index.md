...
This property can be set to change the protocol of the URL. A `":"` is appended to the provided string if not provided. The provided scheme has to be compatible with the rest of the URL to be considered valid. Setting this property updates the element's {{SVGAttr("href")}} attribute with the serialized, absolute URL.
